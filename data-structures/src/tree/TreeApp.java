...
/**
 * https://github.com/dlbunker/ps-data-structure-helpers/blob/master/src/java/
...
public class TreeApp {
	...
	public void searchContacts() {
		// We don't need to populate phone since we only compare on last
		// name
		System.out.println(contacts.contains(new Contact("Fay", "Moro", null))); // should
																					// be
																					// true
		System.out.println(contacts.contains(new Contact("Bob", "Hope", null))); // should
																					// be
																					// false
	}

	public void cleanupContacts() {
		// we just need to remove by last name since that's all we check in
		// the comparable
		contacts.delete(new Contact("Pei", "Rogan", null)); // delete a leaf
															// node
		contacts.delete(new Contact("Porsche", "Gadsden", null)); // delete
																	// a
																	// node
																	// with
																	// a
																	...
																	// node
																	// only
		contacts.delete(new Contact("Annetta", "Coale", null)); // delete a
																// node with
																// a left
																// node only
		contacts.delete(new Contact("Cora", "Bonhomme", null)); // delete a
																// node with
																// both
																// children
		contacts.delete(new Contact("Abe", "Lincoln", null)); // delete the
																// root node
	...
	class Contact implements Comparable<Contact> {
		...
		public int compareTo(Contact other) {
			// For simplicity, we're only going to sort on the contact's
			// last name
